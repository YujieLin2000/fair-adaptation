...
def curve_plot(bestof1, bestof2, nsteps, figsize, logscale=False, endstep=2000, confidence=(5, 95)):
    """Draw mean trajectory plot with percentiles"""
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=figsize)
    for model, item in sorted(bestof1.items()):
        xx = item['steps']
        values = item['kl']
    ...
    for model, item in sorted(bestof2.items()):
        xx = item['steps']
        values = item['kl']
    ...
    ax.axvline(500, linestyle='--', color='black')
    ax.axvline(1500, linestyle='--', color='black')
    ax.grid(True)
    if logscale:
        ax.set_yscale('log')
...
def two_plots(results1, results2, nsteps, plotname1, plotname2, dirname, verbose=False, figsize=(6, 3)):
    print(dirname, plotname1,plotname2,'0000')
    bestof1 = get_best(results1, nsteps)
    bestof2 = get_best(results2, nsteps)
    ...
    selected1 = {
        ...
        for key, item in bestof1.items()
        if key.endswith('_average')}
    selected2 = {
        key[0].capitalize() + key[1:-len('_average')].replace('A', 'X').replace('B', 'Y'): item
        for key, item in bestof2.items()
        if key.endswith('_average')}
    curves, ax1 = curve_plot(selected1, selected2, nsteps, figsize, logscale=False)
    ...
    for style, fig in {'curves': curves}.items():
        for figpath in [
            os.path.join('plots', dirname, f'{style}_{plotname1+plotname2}.pdf')]:
            print("Saving ", figpath,'1111')
...
def all_plot(guess=False, dense=True,
             input_dir='axy_categorical_results', output_dir='AXY/compare_plot',
             figsize=(3.6, 2.2)):
    ...
    for k in [20]:  # [10, 20, 50]:
        ...
        for intervention1,intervention2 in [('X','cause'),('Y','effect')]:
            # 'singlecond', 'gmechanism', 'independent', 'geometric', 'weightedgeo']:
            plotname1 = f'{prior_string}_{intervention1}_k={k}'
            file1 = f'{basefile}_{intervention1}_k={k}.pkl'
            plotname2 = f'{prior_string}_{intervention2}_k={k}'
            file2 = f'{basefile}_{intervention2}_k={k}.pkl'
            filepath1= os.path.join(input_dir, file1)
            filepath2= os.path.join(input_dir, file2)
            print(os.path.abspath(filepath1))
            print(os.path.abspath(filepath2))
            if os.path.isfile(filepath1) and os.path.isfile(filepath2):
                with open(filepath1, 'rb') as fin1,open(filepath2, 'rb') as fin2:
                    results1 = pickle.load(fin1)
                    results2 = pickle.load(fin2)
                    print(1)
                    two_plots(results1,results2,nsteps,
                              plotname1=plotname1,
                              plotname2=plotname2,
                              dirname=output_dir,
                              figsize=figsize)
...
if __name__ == '__main__':
    all_plot(dense=True, input_dir='AXY/compare_result')